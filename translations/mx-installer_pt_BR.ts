<?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE TS>
<TS version="2.1" language="pt_BR">
<context>
    <name>MInstall</name>
    <message>
        <location filename="../minstall.cpp" line="289"/>
        <source>

The disk with the partition you selected for installation is failing.

</source>
        <translation>

O disco com a partição que você selecionou para a instalação está falhando. 
</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="290"/>
        <source>You are strongly advised to abort.
</source>
        <translation>Você é vivamente aconselhado para abortar. </translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="291"/>
        <location filename="../minstall.cpp" line="305"/>
        <source>If unsure, please exit the Installer and run GSmartControl for more information.

</source>
        <translation>Se tiver dúvidas, por favor, saia do Installer e execute GSmartControl para mais informações. </translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="292"/>
        <source>Do you want to abort the installation?</source>
        <translation>Você quer abortar a instalação? </translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="294"/>
        <location filename="../minstall.cpp" line="308"/>
        <location filename="../minstall.cpp" line="488"/>
        <location filename="../minstall.cpp" line="687"/>
        <location filename="../minstall.cpp" line="699"/>
        <location filename="../minstall.cpp" line="715"/>
        <location filename="../minstall.cpp" line="728"/>
        <location filename="../minstall.cpp" line="741"/>
        <location filename="../minstall.cpp" line="986"/>
        <location filename="../minstall.cpp" line="1094"/>
        <location filename="../minstall.cpp" line="1099"/>
        <location filename="../minstall.cpp" line="1124"/>
        <location filename="../minstall.cpp" line="1840"/>
        <location filename="../minstall.cpp" line="1854"/>
        <location filename="../minstall.cpp" line="2551"/>
        <source>Yes</source>
        <translation>Sim</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="294"/>
        <location filename="../minstall.cpp" line="308"/>
        <location filename="../minstall.cpp" line="488"/>
        <location filename="../minstall.cpp" line="687"/>
        <location filename="../minstall.cpp" line="699"/>
        <location filename="../minstall.cpp" line="715"/>
        <location filename="../minstall.cpp" line="728"/>
        <location filename="../minstall.cpp" line="741"/>
        <location filename="../minstall.cpp" line="986"/>
        <location filename="../minstall.cpp" line="1094"/>
        <location filename="../minstall.cpp" line="1099"/>
        <location filename="../minstall.cpp" line="1124"/>
        <location filename="../minstall.cpp" line="1840"/>
        <location filename="../minstall.cpp" line="1854"/>
        <location filename="../minstall.cpp" line="2551"/>
        <source>No</source>
        <translation>Não</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="302"/>
        <source>Smartmon tool output:

</source>
        <translation>Saída da ferramenta SMARTMON: 
</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="303"/>
        <source>The disk with the partition you selected for installation passes the S.M.A.R.T. monitor test (smartctl)
</source>
        <translation>O disco com a partição que você selecionou para a instalação passa no SMART teste de monitor (smartctl) </translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="304"/>
        <source>but the tests indicate it will have a higher than average failure rate in the upcoming year.
</source>
        <translation>mas os testes indicam que ele terá uma taxa de insucesso maior do que a média no próximo ano. </translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="306"/>
        <source>Do you want to continue?</source>
        <translation>Você quer continuar? </translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="340"/>
        <source>Ready to install MX Linux filesystem</source>
        <translation>Pronto para instalar o sistema de arquivos do Linux MX</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="486"/>
        <source>OK to format and use the entire disk (%1) for MX Linux?</source>
        <translation>OK para formatar e usar todo o disco (%1) para o Linux MX?</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="494"/>
        <source>Creating required partitions</source>
        <translation>Criando as partições necessárias</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="569"/>
        <source>Formating EFI System Partition (ESP)</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="607"/>
        <location filename="../minstall.cpp" line="779"/>
        <source>Formatting swap partition</source>
        <translation>Formatando a partição swap</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="616"/>
        <source>Formatting root partition</source>
        <translation>Formatando a partição root</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="679"/>
        <source>You must choose a root partition.
The root partition must be at least 3.5 GB.</source>
        <translation>Você deve escolher uma partição root.
A partição root deve ser de ao menos3,5 GB.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="685"/>
        <source>The partition you selected for root, appears to be a MS-Windows partition.  Are you sure you want to reformat this partition?</source>
        <translation>A partição que você escolheu para root parece ser uma partição tipo MS Windows.  Você tem certeza de que quer reformatá-la?</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="694"/>
        <source>OK to format and destroy all data on 
%1 for the / (root) partition?</source>
        <translation>OK para formatar e apagar todos os dados em
%1 para a partição / (root)?</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="696"/>
        <source>All data on %1 will be deleted, except for /home
OK to continue?</source>
        <translation>Todos os dados em %1 serão eleiminados, excepto em /home
OK para continuar?</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="713"/>
        <source>OK to format and destroy all data on 
%1 for the swap partition?</source>
        <translation>OK para formatar e apagar todos os dados em
%1 para a partição swap?</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="726"/>
        <source>The partition you selected for /home, appears to be a MS-Windows partition.  Are you sure you want to reformat this partition?</source>
        <translation>A partição que você selecionou para /home parece ser do tipo MS Windows.  Você tem certeza de que quer reformatá-la?</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="735"/>
        <source>OK to reuse (no reformat) %1 as the /home partition?</source>
        <translation>OK para reusar (sem reformatar) %1 como partição /home?</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="737"/>
        <source>OK to format and destroy all data on %1 for the /home partition?</source>
        <translation>OK para formatar e apagar todos os dados em
%1 para a partição /home?</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="748"/>
        <source>Preparing required partitions</source>
        <translation>Preparando as partições necessárias</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="799"/>
        <source>Formatting the / (root) partition</source>
        <translation>Formatando a partição / (root)</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="833"/>
        <source>Mounting the /home partition</source>
        <translation>Montando a partição /home</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="848"/>
        <source>Formatting the /home partition</source>
        <translation>Formatando a partição /home</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="890"/>
        <source>Mounting the / (root) partition</source>
        <translation>Montando a partição / (root)</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="917"/>
        <source>Creating system directories</source>
        <translation>Criando diretórios do sistema</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="984"/>
        <source>OK to install GRUB bootloader at %1 ?</source>
        <translation>OK para instalar o gerenciador de boot GRUB em %1?</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="996"/>
        <source>Please wait till GRUB is installed, it might take a couple of minutes.</source>
        <translation>Por favor, aguarde até que o GRUB esteja instalado, isso pode demorar alguns minutos.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1032"/>
        <source>Sorry, installing GRUB failed. This may be due to a change in the disk formatting. You can uncheck GRUB and finish installing MX Linux then reboot to the LiveDVD or LiveUSB and repair the installation with the reinstall GRUB function.</source>
        <translation>Desculpe, a instalação do gerenciador GRUB falhou. Isto pode se dever a uma alteração na formatação do disco. Você pode desmarcar o GRUB e terminar a instalação do Linux MX, e depois reinicilizar com LiveDVD ou LiveUSB e reparar a instalação no disco com a função de reinstalação do GRUB.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1091"/>
        <source>The home directory for %1 already exists.Would you like to reuse the old home directory?</source>
        <translation>O diretório home para %1 já existe. Você gostaria de reutilizar o diretório home antigo?</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1097"/>
        <source>Would you like to save the old home directory
and create a new home directory?</source>
        <translation>Você gostaria de salvar o diretório home antigo
e criar um diretório home novo?</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1113"/>
        <source>Sorry, failed to save old home directory. Before proceeding,
you&apos;ll have to select a different username or
delete a previously saved copy of your home directory.</source>
        <translation>Desculpe, salvar o diretório home antigo falhou. Antes de prosseguir
você deverá selecionar um nome de usuário diferente ou
apagar uma cópia de seu antigo diretório home previamente salva.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1122"/>
        <source>Would you like to delete the old home directory for %1?</source>
        <translation>Você gostaria de apagar o diretório home antigo em %1?</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1132"/>
        <source>Sorry, failed to delete old home directory. Before proceeding, 
you&apos;ll have to select a different username.</source>
        <translation>Desculpe, apagar o diretório home antigo falhou. Antes de prosseguir
você deverá selecionar um nome de usuário diferente.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1139"/>
        <source>You&apos;ve chosen to not use, save or delete the old home directory.
Before proceeding, you&apos;ll have to select a different username.</source>
        <translation>Você escolheu não usar, salvar ou apagar o diretório home antigo.
Antes de prosseguir você deverá selecionar um nome de usuário diferente.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1152"/>
        <source>Sorry, failed to create user directory.</source>
        <translation>Desculpe, a criação do diretório do usuário falhou.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1159"/>
        <source>Sorry, failed to name user directory.</source>
        <translation>Desculpe, a nomeação do diretório do usuário falhou.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1184"/>
        <source>Sorry, failed to save desktop changes.</source>
        <translation>Desculpe, salvar as alterações do ambiente de trabalho falhou.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1192"/>
        <source>Sorry, failed to set ownership of user directory.</source>
        <translation>Desculpe, a atribuição de propriedade do diretório do usuário falhou.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1241"/>
        <source>Sorry, could not encrypt /home/</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1286"/>
        <source>Sorry, unable to set root password.</source>
        <translation>Desculpe, incapaz de estabelecer a senha de root.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1313"/>
        <source>Sorry, unable to set user password.</source>
        <translation>Desculpe, incapaz de estabelecer a senha do usuário.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1326"/>
        <source>The user name needs to be at least
2 characters long. Please select
a longer name before proceeding.</source>
        <translation>O nome do usuário necessita ter pelo
menos 2 caracteres. Por favor, escolha
um nome de usuário mais longo antes de prosseguir.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1332"/>
        <source>The user name needs be lower case and it
cannot contain special characters or spaces.
Please choose another name before proceeding.</source>
        <translation>O nome de usuário necessita estar em minúsculas
e não pode conter caracteres especiais nem espaços.
Por favor, escolha outro nome de usuário antes de prosseguir.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1339"/>
        <location filename="../minstall.cpp" line="1374"/>
        <source>The user password needs to be at least
2 characters long. Please select
a longer password before proceeding.</source>
        <translation>A senha do usuário necessita ter ao menos
2 caracteres. Por favor, escolha uma senha
mais longa antes de prosseguir.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1346"/>
        <location filename="../minstall.cpp" line="1381"/>
        <source>The root password needs to be at least
2 characters long. Please select
a longer password before proceeding.</source>
        <translation>A senha de root necessita ter ao menos
2 caracteres. Por favor, escolha uma senha
mais longa antes de prosseguir.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1355"/>
        <source>Sorry that name is in use.
Please select a different name.
</source>
        <translation>Desculpe, este nome está em uso.
Por favor, escolha um nome diferente.
</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1362"/>
        <source>The user password entries do
not match.  Please try again.</source>
        <translation>As senhas do usuário não combinam.
Por favor, tente de novo.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1368"/>
        <source>The root password entries do
 not match.  Please try again.</source>
        <translation>As senhas de root não combinam.
Por favor, tente de novo.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1403"/>
        <source>Sorry your computer name needs to be
at least 2 characters long. You&apos;ll have to
select a different name before proceeding.</source>
        <translation>Desculpe, o nome da máquina deve ter
ao menos 2 caracteres. Você deve escolher
um nome diferente antes de prosseguir.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1407"/>
        <source>Sorry your computer name contains invalid characters.
You&apos;ll have to select a different
name before proceeding.</source>
        <translation>Desculpe o nome do computador contém caracteres inválidos. 
Você deve escolher um nome diferente antes de prosseguir.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1413"/>
        <source>Sorry your computer domain needs to be at least
2 characters long. You&apos;ll have to select a different
name before proceeding.</source>
        <translation>Desculpe, o domínio da sua máquina deve conter
ao menos 2 caracteres. Você deve escolher um
nome diferente antes de prosseguir.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1417"/>
        <source>Sorry your computer domain contains invalid characters.
You&apos;ll have to select a different
name before proceeding.</source>
        <translation>Desculpe, o domínio da sua máquina contém caracteres inválidos.
Você deve escolher um nome diferente antes de prosseguir.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1426"/>
        <source>Sorry your workgroup needs to be at least
2 characters long. You&apos;ll have to select a different
name before proceeding.</source>
        <translation>Desculpe, seu grupo de trabalho deve ter
ao menos 2 caracteres. Você deve escolher
um nome diferente antes de prosseguir.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1837"/>
        <source>MX Linux installation and configuration is complete.
To use the new installation, reboot without the installation media.

Do you want to reboot now?</source>
        <translation>A instalação e a configuração do Linux MX estão completas.
Para usar a nova instalação reinicialize o computador sem o CD.

Você quer reiniciar agora? </translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1853"/>
        <source>The installation and configuration is incomplete.
Do you really want to stop now?</source>
        <translation>A instalação e a configuração estão incompletas.
Você realmente deseja interromper agora?</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1928"/>
        <source>&lt;p&gt;&lt;b&gt;General Instructions&lt;/b&gt;&lt;br/&gt;BEFORE PROCEEDING, CLOSE ALL OTHER APPLICATIONS.&lt;/p&gt;&lt;p&gt;On each page, please read the instructions, make your selections, and then click on Next when you are ready to proceed. You will be prompted for confirmation before any destructive actions are performed.&lt;/p&gt;&lt;p&gt;MX Linux requires about 3.5 GB of space. 5 GB or more is preferred. You can use the entire disk or you can put MX Linux on existing partitions. &lt;/p&gt;&lt;p&gt;If you are running Mac OS or Windows OS (from Vista onwards), you may have to use that system&apos;s software to set up partitions and boot manager before installing MX Linux.&lt;/p&gt;&lt;p&gt;The ext2, ext3, ext4, jfs, xfs, btrfs and reiserfs Linux filesystems are supported and ext4 is recommended.&lt;/p&gt;</source>
        <translation>&lt;p&gt;&lt;b&gt;Instruções Gerais&lt;/b&gt;&lt;br/&gt;ANTES DE PROSSEGUIR, FECHE TODAS AS OUTRAS APLICAÇÕES. SE VOCÊ ESTÁ USANDO O DESKTOP 3D, VOCÊ DEVE SAIR DA SESSÃO E ENTRAR NOVAMENTE NUMA SESSÃO REGULAR DO KDE.&lt;/p&gt;&lt;p&gt;Por favor, leia as instruções em cada página, faça suas escolhas e então clique em Próximo quando estiver pronto para prosseguir. Será pedida confirmação antes da efetuação de quaisquer ações destrutivas.&lt;/p&gt;&lt;p&gt;Versões leves do Linux MX necessitam de cerca de 3,5 GB. 5 GB ou mais é preferível. Você pode usar todo o disco ou pode colocar o Linux MX em partições pré-existentes. &lt;/p&gt;&lt;p&gt;Se você está usando um equipamento do tipo PC, execute o Gparted a partir daqui se precisar modificar partições para uma instalação personalizada. Se você está usando um equipamento Apple, você jamais deve usar o Gparted no seu disco de boot. Ao invés disso, você deve configurar suas partições e o gerenciador de inicialização no OSX antes de instalar o Linux MX.&lt;/p&gt;&lt;p&gt;São suportados os sistemas de arquivos Linux ext2, ext3, ext4, jfs, xfs, btrfs e reiserfs e ext4 é o recomendado.&lt;/p&gt;</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1939"/>
        <source>&lt;p&gt;&lt;b&gt;Limitations&lt;/b&gt;&lt;br/&gt;Remember, this software is provided AS-IS with no warranty what-so-ever. It&apos;s solely your responsibility to backup your data before proceeding.&lt;/p&gt;&lt;p&gt;&lt;b&gt;Choose Partitions&lt;/b&gt;&lt;br/&gt;MX Linux requires a root partition. The swap partition is optional but highly recommended. If you want to use the Suspend-to-Disk feature of MX Linux, you will need a swap partition that is larger than your physical memory size.&lt;/p&gt;&lt;p&gt;If you choose a separate /home partition it will be easier for you to upgrade in the future, but this will not be possible if you are upgrading from an installation that does not have a separate home partition.&lt;/p&gt;&lt;p&gt;&lt;b&gt;Upgrading&lt;/b&gt;&lt;br/&gt;To upgrade from an existing Linux installation, select the same home partition as before and check the preference to preserve data in /home.&lt;/p&gt;&lt;p&gt;If you are preserving an existing /home directory tree located on your root partition, the installer will not reformat the root partition. As a result, the installation will take much longer than usual.&lt;/p&gt;&lt;p&gt;&lt;b&gt;Preferred Filesystem Type&lt;/b&gt;&lt;br/&gt;For MX Linux, you may choose to format the partitions as ext2, ext3, ext4, jfs, xfs, btrfs or reiser. &lt;/p&gt;&lt;p&gt;&lt;b&gt;Bad Blocks&lt;/b&gt;&lt;br/&gt;If you choose ext2, ext3 or ext4 as the format type, you have the option of checking and correcting for bad blocks on the drive. The badblock check is very time consuming, so you may want to skip this step unless you suspect that your drive has bad blocks.&lt;/p&gt;</source>
        <translation>&lt;p&gt;&lt;b&gt;Limitações&lt;/b&gt;&lt;br/&gt;Lembre-se de que este programa é disponibilizado NO ESTÁDO, sem qualquer garantia. É da sua exclusiva responsabilidade fazer backup de seus dados antes de prosseguir.&lt;/p&gt;&lt;p&gt;&lt;b&gt;Escolher Partições&lt;/b&gt;&lt;br/&gt;O Linux MX requer uma partição root. A partição swap é opcional, mas é altamente recomendada. Se você quiser usar a função &apos;Hibernar&apos; do Linux MX, precisará de uma partição swap maior do que a memória física do computador.&lt;/p&gt;&lt;p&gt;Se você escolher uma partição /home separada, será mais simples, no futuro, fazer o upgrade do sistema para uma versão mais recente. Mas se você está atualizando uma instalação que não tenha a partição /home separada, já não será possível fazer a separação. &lt;/p&gt;&lt;p&gt;&lt;b&gt;Upgrade&lt;/b&gt;&lt;br/&gt;Para instalar uma nova versão do Linux MX sobre qualquer sistema Linux existente, seleccione a partição home existente e marque a opção de preservar os dados em /home.&lt;/p&gt;&lt;p&gt;Se você está preservando a árvore do diretório /home localizado na partição root, o instalador não reformatará essa partição. Como resultado, a instalação demorará muito mais do que o normal.&lt;/p&gt;&lt;p&gt;&lt;b&gt;Sistema de Arquivos Preferido&lt;/b&gt;&lt;br/&gt;No Linux MX, você poderá optar por formatar as partições como ext2, ext3, ext4, jfs, xfs, btrfs ou reiserfs. &lt;/p&gt;&lt;p&gt;&lt;b&gt;Blocos Defeituosos&lt;/b&gt;&lt;br/&gt;Se escolher ext2, ext3 ou ext4, terá a opção de checagem e correção de blocos defeituosos no disco. Essa checagem é muito demorada, por isso talvez você queira pular essa etapa se não suspeiter que o disco tenha blocos defeituosos.&lt;/p&gt;</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1953"/>
        <source>Returning to Step 1 to select another disk.</source>
        <translation>Retornando ao Passo 1.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1957"/>
        <source>&lt;p&gt;&lt;b&gt;Special Thanks&lt;/b&gt;&lt;br/&gt;Thanks to everyone who has chosen to support MX Linux with their time, money, suggestions, work, praise, ideas, promotion, and/or encouragement.&lt;/p&gt;&lt;p&gt;Without you there would be no MX Linux.&lt;/p&gt;&lt;p&gt;MX Dev Team&lt;/p&gt;</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2068"/>
        <location filename="../minstall.cpp" line="2080"/>
        <source>Next</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2768"/>
        <source>&lt;p&gt;&lt;b&gt;Support MX Linux&lt;/b&gt;&lt;br/&gt;MX Linux is supported by people like you. Some help others at the support forum - https://forum.mxlinux.org, or translate help files into different languages, or make suggestions, write documentation, or help test new software.&lt;/p&gt;</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2783"/>
        <source>&lt;p&gt;&lt;b&gt;Keep Your Copy of MX Linux up-to-date&lt;/b&gt;&lt;br/&gt;For MX Linux information and updates please visit https://mxlinux.org&lt;/p&gt;</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <source>&lt;p&gt;&lt;b&gt;Special Thanks&lt;/b&gt;&lt;br/&gt;Thanks to everyone who has chosen to support MX Linux with their time, money, suggestions, work, praise, ideas, promotion, and/or encouragement.&lt;/p&gt;&lt;p&gt;Without you there would be no MX Linux.&lt;/p&gt;&lt;p&gt;anticapitalista, MX Community&lt;/p&gt;</source>
        <translation type="vanished">&lt;p&gt;&lt;b&gt;Agradecimentos&lt;/b&gt;&lt;br/&gt;Agradecemos a todos os que decidiram apoiar o Linux MX com seu tempo, dinheiro, sugestões, trabalho, elogios, ideias, promoção e/ou encorajamento.&lt;/p&gt;&lt;p&gt;Sem eles não haveria o Linux MX.&lt;/p&gt;&lt;p&gt;anticapitalista e Comunidade MX&lt;/p&gt;</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1960"/>
        <source>&lt;p&gt;&lt;b&gt;Installation in Progress&lt;/b&gt;&lt;br/&gt;MX Linux is installing.  For a fresh install, this will probably take 3-20 minutes, depending on the speed of your system and the size of any partitions you are reformatting.&lt;/p&gt;&lt;p&gt;If you click the Abort button, the installation will be stopped as soon as possible.&lt;/p&gt;</source>
        <translation>&lt;p&gt;&lt;b&gt;Instalação em Andamento&lt;/b&gt;&lt;br/&gt;O Linux MX está sendo instalado. Uma instalação original geralmente leva de 3 a 20 min, dependendo da velocidade de seu equipamento e do tamanho das partições a serem formatadas.&lt;/p&gt;&lt;p&gt;Se você clicar no botão Abortar, a instalação será interrompida o quanto antes.&lt;/p&gt;</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1972"/>
        <source>Failed to create required partitions.
Returning to Step 1.</source>
        <translation>A criação das partições necessárias falhou.
Retornando ao Passo 1.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1981"/>
        <source>Failed to prepare chosen partitions.
Returning to Step 1.</source>
        <translation>O preparo das partições selecionadas falhou.
Retornando ao Passo 1.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="1994"/>
        <source>&lt;p&gt;&lt;b&gt;Select Boot Method&lt;/b&gt;&lt;br/&gt;MX Linux uses the GRUB bootloader to boot MX Linux and MS-Windows. &lt;p&gt;By default GRUB2 is installed in the Master Boot Record or ESP (EFI System Partition for 64-bit UEFI boot systems) of your boot drive and replaces the boot loader you were using before. This is normal.&lt;/p&gt;&lt;p&gt;If you choose to install GRUB2 at root instead, then GRUB2 will be installed at the beginning of the root partition. This option is for experts only.&lt;/p&gt;&lt;p&gt;If you uncheck the Install GRUB box, GRUB will not be installed at this time. This option is for experts only.&lt;/p&gt;</source>
        <translation>&lt;p&gt;&lt;b&gt;Opções de Inicialização&lt;/b&gt;&lt;br/&gt;O Linux MX usa o gerenciador de boot GRUB para inicializar o Linux MX e o MS-Windows. &lt;p&gt;Por padrão o GRUB2 será instalado no MBR (Master Boot Record), ou na ESP ([EFI System Partition] no caso de sistemas de 64-bit que inicializam por UEFI), de seu disco de boot e substituirá o gerenciador de boot instalado. Isto é normal.&lt;/p&gt;&lt;p&gt;Se optar por instalar o GRUB2 na root, o GRUB2 será instalado no começo da partição root. Usuários pouco experientes NÃO devem usar esta opção.&lt;/p&gt;&lt;p&gt;Se desmarcar a caixa &apos;Instalar GRUB&apos;, o GRUB não será instalado agora. Usuários pouco experientes NÃO devem optar por desmarcar esta caixa.&lt;/p&gt;</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2003"/>
        <source>&lt;p&gt;&lt;b&gt;Common Services to Enable&lt;/b&gt;&lt;br/&gt;Select any of these common services that you might need with your system configuration and the services will be started automatically when you start MX Linux.&lt;/p&gt;</source>
        <translation>&lt;p&gt;&lt;b&gt;Habilitar Serviços Comuns&lt;/b&gt;&lt;br/&gt;Selecione quaisquer serviços de que possa precisar e eles serão iniciados automaticamente quando o Linux MX inicializar.&lt;/p&gt;</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2010"/>
        <source>&lt;p&gt;&lt;b&gt;Computer Identity&lt;/b&gt;&lt;br/&gt;The computer name is a common unique name which will identify your computer if it is on a network. The computer domain is unlikely to be used unless your ISP or local network requires it.&lt;/p&gt;&lt;p&gt;The computer and domain names can contain only alphanumeric characters, dots, hyphens. They cannot contain blank spaces, start or end with hyphens&lt;/p&gt;&lt;p&gt;The SaMBa Server needs to be activated if you want to use it to share some of your directories or printer with a local computer that is running MS-Windows or Mac OSX.&lt;/p&gt;</source>
        <translation>&lt;p&gt;&lt;b&gt;Identidade da Máquina&lt;/b&gt;&lt;br/&gt;O nome da máquina é um nome específico que permitirá identificar seu computador em uma rede. O domínio do computador dificilmente será usado a menos que seu provedor de Internet ou rede local o exijam.&lt;/p&gt;&lt;p&gt;Os nomes de computador e domínio podem conter apenas caracteres alfanuméricos, pontos, hífens. Eles não podem conter espaços em branco, começar ou terminar com hífens.&lt;/p&gt;&lt;p&gt;O Servidor Samba deve ser ativado se você quiser utilizá-lo para compartilhar alguns de seus diretórios ou uma impressora com um computador local que esteja rodando MS-Windows ou Mac OSX.&lt;/p&gt;</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2021"/>
        <source>&lt;p&gt;&lt;b&gt;Localization Defaults&lt;/b&gt;&lt;br/&gt;Set the default keyboard and locale. These will apply unless they are overridden later by the user.&lt;/p&gt;&lt;p&gt;&lt;b&gt;Configure Clock&lt;/b&gt;&lt;br/&gt;If you have an Apple or a pure Unix computer, by default the system clock is set to GMT or Universal Time. To change, check the box for &apos;System clock uses LOCAL.&apos;&lt;/p&gt;&lt;p&gt;&lt;b&gt;Timezone Settings&lt;/b&gt;&lt;br/&gt;The system boots with the timezone preset to GMT/UTC. To change the timezone, after you reboot into the new installation, right click on the clock in the Panel and select Properties.&lt;/p&gt;&lt;p&gt;&lt;b&gt;Service Settings&lt;/b&gt;&lt;br/&gt;Most users should not change the defaults. Users with low-resource computers sometimes want to disable unneeded services in order to keep the RAM usage as low as possible. Make sure you know what you are doing! </source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2031"/>
        <source>&lt;p&gt;&lt;b&gt;Default User Login&lt;/b&gt;&lt;br/&gt;The root user is similar to the Administrator user in some other operating systems. You should not use the root user as your daily user account. Please enter the name for a new (default) user account that you will use on a daily basis. If needed, you can add other user accounts later with MX User Manager. &lt;/p&gt;&lt;p&gt;&lt;b&gt;Passwords&lt;/b&gt;&lt;br/&gt;Enter a new password for your default user account and for the root account. Each password must be entered twice.&lt;/p&gt;</source>
        <translation>&lt;p&gt;&lt;b&gt;Login do Usuário Padrão&lt;/b&gt;&lt;br/&gt;O usuário root é similar ao usuário Administrador em outros sistemas operacionais. Você não deve usá-lo como sua conta do dia-a-dia. Por favor, informe o nome para uma nova conta de usuário (padrão) que será usada no dia-a-dia. Se necessário for, você poderá acrescentar novos usuários posteriormente. &lt;/p&gt;&lt;p&gt;&lt;b&gt;Senhas&lt;/b&gt;&lt;br/&gt;Informe novas senhas para as contas de usuário padrão e root. Cada senha deve ser teclada duas vezes.&lt;/p&gt;</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2042"/>
        <source>&lt;p&gt;&lt;b&gt;Congratulations!&lt;/b&gt;&lt;br/&gt;You have completed the installation of </source>
        <translation>&lt;p&gt;&lt;b&gt;Parabéns&lt;/b&gt;&lt;br/&gt;Você completou a instalação do </translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2042"/>
        <source>&lt;/p&gt;&lt;p&gt;&lt;b&gt;Finding Applications&lt;/b&gt;&lt;br/&gt;There are hundreds of excellent applications installed with MX Linux. The best way to learn about them is to browse through the Menu and try them. Many of the apps were developed specifically for the Xfce environment. These are shown in the main menus. &lt;p&gt;In addition MX Linux includes many standard Linux applications that are run only from the command line and therefore do not show up in the Menu.&lt;/p&gt;</source>
        <translation>&lt;/p&gt;&lt;p&gt;&lt;b&gt;Encontrar Aplicativos&lt;/b&gt;&lt;br/&gt;Com o Linux MX são instalados centenas de excelentes aplicativos. A melhor maneira de se familiarizar com eles é navegar pelo Menu e experimentá-los. Muitos dos aplicativos foram desenvolvidos especificamente para o ambiente Xfce. Esses são acedidos pelos menus principais. &lt;p&gt;O Linux MX inclui ainda muitos aplicativos Linux padrão que são executados apenas pelo terminal de linha de comando e, portanto, não aparecem no Menu.&lt;/p&gt;</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2054"/>
        <source>&lt;p&gt;&lt;b&gt;Enjoy using MX Linux!&lt;/b&gt;&lt;/p&gt;</source>
        <translation>&lt;p&gt;&lt;b&gt;Aproveite bem o Linux MX!&lt;/b&gt;&lt;/p&gt;</translation>
    </message>
    <message>
        <source>Next &gt;</source>
        <translation type="vanished">Próximo &gt;</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2078"/>
        <source>Finish</source>
        <translation>Encerrar</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2143"/>
        <source>Administration</source>
        <translation>Administração</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2149"/>
        <source>Runs commands periodically</source>
        <translation>Executa os comandos periodicamente </translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2159"/>
        <source>Time-based job scheduler</source>
        <translation>Definidor de acções a executar a horário</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2169"/>
        <source>Execute a command as another user</source>
        <translation>Executar um comando como usuário diferente</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2178"/>
        <source>Hardware</source>
        <translation>Hardware</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2183"/>
        <source>CPU frequency, irqbalance</source>
        <translation>Frequêcia da CPU, irqbalance</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2193"/>
        <source>S.M.A.R.T. monitoring tools</source>
        <translation>Ferramentas de monitorização S.M.A.R.T.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2203"/>
        <source>Advanced Configuration and Power Interface event daemon</source>
        <translation>Daemon (processo oculto) de eventos da Interface Avançada de Configuração e Energia (ACPI)</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2213"/>
        <source>HDD temperature monitor</source>
        <translation>Monitor de temperatura do disco rígido (HDD)</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2223"/>
        <source>Generates key events for ACPI scripts</source>
        <translation>Gera eventos-chave para scripts ACPI</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2233"/>
        <source>Message bus daemon</source>
        <translation>Daemon (processo oculto) de transporte de menssagens</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2243"/>
        <source>Mouse event server</source>
        <translation>Servidor de eventos do mouse</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2253"/>
        <source>Scanner daemon</source>
        <translation>Scanner daemon </translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2263"/>
        <source>Random number generator</source>
        <translation>Gerador de números aleatórios</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2273"/>
        <source>Enables programs to publish and discover services and hosts</source>
        <translation>Habilitar programas a publicar e a descobrir serviços e hospedeiros.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2282"/>
        <source>Networking</source>
        <translation>Rede</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2288"/>
        <source>Network connection</source>
        <translation>Conectar rede</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2298"/>
        <source>Secure Shell</source>
        <translation>Secure Shell </translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2308"/>
        <source>Mail filter</source>
        <translation>Filtro de correio</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2318"/>
        <source>Network File System</source>
        <translation>Sistema de Arquivos de Rede</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2332"/>
        <source>VPN program that creates secure connections</source>
        <translation>Programa de redes privadas virtuais que cria conexões seguras</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2342"/>
        <source>Bluetooth</source>
        <translation>Bluetooth </translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2352"/>
        <source>File-copying tool</source>
        <translation>Ferramenta de cópia de arquivos</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2361"/>
        <source>Printing</source>
        <translation>Impressão</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2367"/>
        <source>Linux and OS X printer service</source>
        <translation>Serviço de impressoras para Linux e OS X</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2550"/>
        <source>MX Linux is installing, are you 
sure you want to Close now?</source>
        <translation>O Linux MX está sendo instalado, você 
tem certeza de que quer Encerrar agora?</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2577"/>
        <source>Deleting old system</source>
        <translation>Apagando o sistema antigo</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2586"/>
        <source>Failed to delete old MX Linux on destination.
Returning to Step 1.</source>
        <translation>Apagar o Linux MX antigo na destinação falhou.
Retornando ao Passo 1.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2612"/>
        <source>Copying new system</source>
        <translation>Copiando o novo sistema</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2636"/>
        <source>Fixing configuration</source>
        <translation>Reparando a configuração</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2727"/>
        <source>Failed to write MX Linux to destination.
Returning to Step 1.</source>
        <translation>A escrita do Linux MX na destinação falhou.
Retornando ao Passo 1.</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2754"/>
        <source>&lt;p&gt;&lt;b&gt;Getting Help&lt;/b&gt;&lt;br/&gt;Basic information about MX Linux is at https://mxlinux.org There are volunteers to help you at the MX forum, https://forum.mxlinux.org &lt;/p&gt;&lt;p&gt;If you ask for help, please remember to describe your problem and your computer in some detail. Usually statements like &apos;it didn&apos;t work&apos; are not helpful.&lt;/p&gt;</source>
        <translation>&lt;p&gt;&lt;b&gt;Obter Ajuda&lt;/b&gt;&lt;br/&gt;Informação básica sobre o Linux MX em https://mxlinux.org. Há voluntários que prestam ajuda no fórum do MX, https://forum.mxlinux.org. &lt;/p&gt;&lt;p&gt;Se você solicitar ajuda, por favor se lembre de pormenorizar suficientemente a descrição de seu problema e a informação sobre seu computador. Por regra, afirmações como &apos;não funcionou&apos; não são muito úteis para quem presta ajuda.&lt;/p&gt;</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2762"/>
        <source>&lt;p&gt;&lt;b&gt;Repairing Your Installation&lt;/b&gt;&lt;br/&gt;If MX Linux stops working from the hard drive, sometimes it&apos;s possible to fix the problem by booting from LiveDVD or LiveUSB and running one of the utilities in MX Tools or by using one of the regular Linux tools to repair the system.&lt;/p&gt;&lt;p&gt;You can also use your MX Linux LiveDVD or LiveUSB to recover data from MS-Windows systems!&lt;/p&gt;</source>
        <translation>&lt;p&gt;&lt;b&gt;Reparando sua Instalação do Linux MX&lt;/b&gt;&lt;br/&gt;Se o Linux MX deixar de funcionar no disco rígido, às vezes é possível reparar o problema fazendo boot por LiveDVD ou LiveUSB e executando alguma das ferramentas da Configuração do Sistema, ou usando uma das ferramentas comuns do Linux.&lt;/p&gt;&lt;p&gt;Você pode também utilizar o LiveDVD ou LiveUSB do Linux MX para recuperar dados de sistemas MS-Windows!&lt;/p&gt;</translation>
    </message>
    <message>
        <source>&lt;p&gt;&lt;b&gt;Support MX Linux&lt;/b&gt;&lt;br/&gt;MX Linux is supported by people like you. Some help others at the support forum - https://forum.mxlinux.org, - http://antix.freeforums.org, or translate help files into different languages, or make suggestions, write documentation, or help test new software.&lt;/p&gt;</source>
        <translation type="vanished">&lt;p&gt;&lt;b&gt;Apoie o Linux MX&lt;/b&gt;&lt;br/&gt;O Linux MX é suportado por pessoas como você. Algumas prestam ajuda a quem a solicita nos fóruns de apoio - https://forum.mxlinux.org e http://antix.freeforums.org, outras traduzem ficheiros da ajuda para diferentes idiomas, fazem sugestões, elaboram documentação ou ajudam na testagem de novos programas.&lt;/p&gt;</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2775"/>
        <source>&lt;p&gt;&lt;b&gt;Adjusting Your Sound Mixer&lt;/b&gt;&lt;br/&gt;MX Linux attempts to configure the sound mixer for you but sometimes it will be necessary for you to turn up volumes and unmute channels in the mixer in order to hear sound.&lt;/p&gt; &lt;p&gt;The mixer shortcut is located in the menu. Click on it to open the mixer. &lt;/p&gt;</source>
        <translation>&lt;p&gt;&lt;b&gt;Ajustando o Misturador de Som&lt;/b&gt;&lt;br/&gt;O Linux MX procura configurar o misturador de som para você, mas às vezes é necessário que você aumente o volume ou retire do mudo canais no misturador para que possa ouvir o som.&lt;/p&gt; &lt;p&gt;O atalho ao misturador está localizado na bandeja. Clique nele para abrir o misturador.&lt;/p&gt;</translation>
    </message>
    <message>
        <source>&lt;p&gt;&lt;b&gt;Keep Your Copy of MX Linux up-to-date&lt;/b&gt;&lt;br/&gt;For MX Linux information and updates please visit https://mxlinux.org or http://antix.freeforums.org&lt;/p&gt;</source>
        <translation type="vanished">&lt;p&gt;&lt;b&gt;Mantenha seu Linux MX atualizado&lt;/b&gt;&lt;br/&gt;Para informações e atualizações do Linux MX, visite https://mxlinux.org ou http://antix.freeforums.org&lt;/p&gt;</translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2803"/>
        <source>This option also encyrpts /swap, which will render the swap partition unable to be shared with other installed operating systems.</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../minstall.cpp" line="2804"/>
        <source>OK</source>
        <translation>OK</translation>
    </message>
</context>
<context>
    <name>MeInstall</name>
    <message>
        <location filename="../meinstall.ui" line="26"/>
        <source>MeInstall</source>
        <translation>MeInstall</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="47"/>
        <source>Close</source>
        <translation>Encerrar</translation>
    </message>
    <message>
        <source>Next &gt;</source>
        <translation type="vanished">Próximo &gt;</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="80"/>
        <source>Alt+N</source>
        <translation>Alt+N</translation>
    </message>
    <message>
        <source>&lt; Back</source>
        <translation type="vanished">&lt; Voltar</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="72"/>
        <source>Next</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="90"/>
        <source>Back</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="98"/>
        <source>Alt+K</source>
        <translation>Alt+K</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="145"/>
        <source>Terms of Use</source>
        <translation>Termos de Uso</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="199"/>
        <source>MX Linux is an independent Linux distribution based on Debian Stable

MX Linux uses some components from MEPIS Linux which are released under an Apache free license. Some MEPIS components have been modified for MX Linux.

Enjoy using MX Linux.</source>
        <translation>O Linux MX é uma distribuição Linux independente baseada no Debian Stable

O Linux MX usa alguns componentes do Linux MEPIS que estão sob a licença &apos;Apache free&apos;. Alguns componentes do MEPIS foram modificados para o Linux MX.

Aproveite bem o Linux MX.</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="241"/>
        <source>1c. Select type of installation</source>
        <translation>1c. Selecione o tipo de instalação</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="287"/>
        <source>Leave free space up to:</source>
        <translation>Deixar espaço livre até:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="322"/>
        <source>MB </source>
        <translation>MB </translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="338"/>
        <source>Auto-install using entire disk </source>
        <translation>Autoinstalação usando o disco inteiro </translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="354"/>
        <source>Custom install on existing partitions</source>
        <translation>Instalação personalizada em partições existentes</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="373"/>
        <source>1a. Choose disk for installation</source>
        <translation>1a. Escolha o disco para a instalação</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="413"/>
        <source>Use disk:</source>
        <translation>Usar o disco:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="432"/>
        <source>1b. Rearrange disk partitions (optional)</source>
        <translation>1b. Rearranje as partições do disco (opcional)</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="459"/>
        <source>Run partition tool...</source>
        <translation>Execute a ferramenta de partição...</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="472"/>
        <source>Modify partitions:</source>
        <translation>Modificar partições:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="536"/>
        <source>2b. Preferences</source>
        <translation>2b. Preferências</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="563"/>
        <source>Preserve data in /home (if upgrading)</source>
        <translation>Preservar dados em /home (se estiver atualizando)</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="598"/>
        <source>Filesystem type:</source>
        <translation>Tipo de sistema de arquivos:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="624"/>
        <source>Change default / (root) partition label:</source>
        <translation>Alterar padrão de label (rótulo) da partição / (root):</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="638"/>
        <source>ext4</source>
        <translation>ext4</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="643"/>
        <source>ext3</source>
        <translation>ext3</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="648"/>
        <source>ext2</source>
        <translation>ext2</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="653"/>
        <source>jfs</source>
        <translation>jfs</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="658"/>
        <source>xfs</source>
        <translation>xfs</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="663"/>
        <source>btrfs</source>
        <translation>btrfs</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="668"/>
        <source>reiserfs</source>
        <translation>reiserfs</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="679"/>
        <source>Change default /home partition label:</source>
        <translation>Alterar rótulo de partição padrão / home:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="698"/>
        <source>Check for badblocks (takes longer)</source>
        <translation>Verificar blocos defeituosos (demora mais)</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="717"/>
        <source>2a. Choose partitions</source>
        <translation>2a. Escolha as partições</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="747"/>
        <source>/ (root) partition:</source>
        <translation>Partição / (root):</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="763"/>
        <source>swap partition:</source>
        <translation>Partição swap:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="779"/>
        <source>/home partition:</source>
        <translation>Partição /home:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="845"/>
        <location filename="../meinstall.ui" line="1209"/>
        <source>root</source>
        <translation>root</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="894"/>
        <source>3. Installation in progress</source>
        <translation>3. Instalação em andamento</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="937"/>
        <source>Abort</source>
        <translation>Abortar</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="940"/>
        <location filename="../meinstall.ui" line="1158"/>
        <source>Alt+A</source>
        <translation>Alt+A</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="979"/>
        <source>Ready to install MX Linux filesystem</source>
        <translation>Pronto para instalar o sistema de arquivos do Linux MX</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1001"/>
        <source>Tips</source>
        <translation>Dicas</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1075"/>
        <source>4. Select Boot Method</source>
        <translation>4. Selecione o Método de Inicialização</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1108"/>
        <source>MBR</source>
        <translation>MBR</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1111"/>
        <source>Alt+B</source>
        <translation>Alt+B</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1139"/>
        <source>Install on:</source>
        <translation>Instalar em:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1155"/>
        <source>Install GRUB for MX Linux and Windows</source>
        <translation>Instalar o GRUB para o Linux MX e o Windows</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1177"/>
        <source>System boot disk:</source>
        <translation>Disco de boot do sistema:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1222"/>
        <source>EFI System Partition</source>
        <translation>Partição de Sistema EFI</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1225"/>
        <source>ESP</source>
        <translation>ESP</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1295"/>
        <source>Common Services to Enable</source>
        <translation>Ativação de Serviços Comuns</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1335"/>
        <source>Service</source>
        <translation>Serviço</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1340"/>
        <source>Description</source>
        <translation>Descrição</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1376"/>
        <source>5. Computer Network Names</source>
        <translation>5. Nome da Máquina para Redes</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1403"/>
        <source>Workgroup</source>
        <translation>Grupo de trabalho:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1416"/>
        <source>Workgroup:</source>
        <translation>Grupo de trabalho:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1432"/>
        <source>SaMBa Server for MS Networking</source>
        <translation>Servidor SaMBa para redes MS</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1448"/>
        <source>example.dom</source>
        <translation>exemplo.dom</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1461"/>
        <source>Computer domain:</source>
        <translation>Domínio do computador:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1477"/>
        <source>mx1</source>
        <translation>mx1</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1493"/>
        <source>Computer name:</source>
        <translation>Nome da máquina:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1566"/>
        <source>6d. Service Settings (advanced)</source>
        <translation>6d. Configurações de serviço (avançado)</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1584"/>
        <source>Adjust which services should run at startup</source>
        <translation>Ajuste que serviços devem executar na inicialização</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1587"/>
        <source>View</source>
        <translation>Vista </translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1616"/>
        <source>6b. Configure Clock</source>
        <translation>6b. Configurar o Relógio</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1644"/>
        <source>Format:</source>
        <translation>Formato:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1661"/>
        <source>System clock uses LOCAL</source>
        <translation>Relógio do sistema usa LOCAL</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1690"/>
        <source>6a. Localization Defaults</source>
        <translation>6a. Padrões de Localização</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1720"/>
        <source>Locale:</source>
        <translation>Localização:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1759"/>
        <source>Keyboard:</source>
        <translation>Teclado:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1778"/>
        <source>6c. Timezone Settings</source>
        <translation>6c. Selecione seu fuso horário</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1790"/>
        <source>Timezone:</source>
        <translation>Fuso horário:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1851"/>
        <source>7b. Root (administrator) Account</source>
        <translation>7b. Conta Root (administrador)</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1922"/>
        <source>Confirm root password:</source>
        <translation>Confirme a senha de root:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1938"/>
        <source>Root password:</source>
        <translation>Senha de root:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="1970"/>
        <source>7a. Default User Account</source>
        <translation>7a. Conta do Usuário Padrão</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="2050"/>
        <source>username</source>
        <translation>nome do usuário</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="2063"/>
        <source>Confirm user password:</source>
        <translation>Confirme a senha do usuário:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="2079"/>
        <source>Default user password:</source>
        <translation>Senha do usuário padrão:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="2095"/>
        <source>Default user login name:</source>
        <translation>Nome de autenticação do usuário padrão:</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="2114"/>
        <source>Autologin</source>
        <translation>Autologin</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="2127"/>
        <source>Show passwords</source>
        <translation>Mostrar senhas</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="2140"/>
        <source>Desktop modifications made in the live environment will be carried over to the installed OS</source>
        <translation>Alterações à área de trabalho feitas no ambiente live serão transportadas para o sistema operacional instalado no disco rígido</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="2143"/>
        <source>Save live desktop changes</source>
        <translation>Salve as alterações do ambiente de trabalho ao vivo</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="2156"/>
        <source>Encrypt /home</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="2188"/>
        <source>Reminders</source>
        <translation>Lembretes</translation>
    </message>
    <message>
        <location filename="../meinstall.ui" line="2236"/>
        <source>Support MX Linux

MX Linux is supported by people like you. Some help others at the support forum - https://forum.mxlinux.org, or translate help files into different languages, or make suggestions, write documentation, or help test new software.</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <source>Support MX Linux

MX Linux is supported by people like you. Some help others at the support forum - https://forum.mxlinux.org, - http://antix.freeforums.org or translate help files into different languages, or make suggestions, write documentation, or help test new software.</source>
        <translation type="vanished">Apoie o MX Linux

O Linux MX é suportado por pessoas como você. Algumas prestam ajuda a quem a solicita nos fóruns de apoio - https://forum.mxlinux.org e http://antix.freeforums.org - outras traduzem ficheiros da ajuda para diferentes línguas, fazem sugestões, elaboram documentação ou ajudam a testar novos programas.</translation>
    </message>
</context>
<context>
    <name>MeMain</name>
    <message>
        <location filename="../memain.ui" line="26"/>
        <source>MX Linux Install</source>
        <translation>Instalador do Linux MX </translation>
    </message>
    <message>
        <location filename="../memain.ui" line="76"/>
        <source>Help</source>
        <translation>Ajuda</translation>
    </message>
</context>
<context>
    <name>QApplication</name>
    <message>
        <location filename="../app.cpp" line="47"/>
        <source>The installer won&apos;t launch because it appears to be running already in the background.

Please close it if possible, or run &apos;pkill minstall&apos; in terminal.</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../app.cpp" line="55"/>
        <source>You are running 32bit OS started in UEFI mode, the system will not be able to boot unless you select Legacy Boot or similar at restart.
We recommend you quit now and restart in Legacy Boot

Do you want to continue the installation?</source>
        <translation type="unfinished"></translation>
    </message>
    <message>
        <location filename="../app.cpp" line="59"/>
        <source>Yes</source>
        <translation>Sim</translation>
    </message>
    <message>
        <location filename="../app.cpp" line="59"/>
        <source>No</source>
        <translation>Não</translation>
    </message>
    <message>
        <location filename="../app.cpp" line="73"/>
        <source>You must run this app as root.</source>
        <translation>Você deve executar este programa como root.</translation>
    </message>
</context>
</TS>
